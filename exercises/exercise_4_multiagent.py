...
Phân tích khía cạnh quyền riêng tư trong câu hỏi sau:

Câu hỏi: {state['question']}
Phân tích pháp lý: {state.get('law_analysis', 'N/A')}

Tập trung: GDPR, rò rỉ dữ liệu (data breach), quyền của chủ thể dữ liệu, và các biện pháp bảo vệ."""
...
def aggregate_results(state: State) -> dict:
    ...
    if state.get("privacy_analysis"):
        sections.append(f"🛡️ PHÂN TÍCH QUYỀN RIÊNG TƯ:\n{state['privacy_analysis']}")
...
def build_graph() -> StateGraph:
    ...
    # Use check_routing as a conditional edge from law_agent
